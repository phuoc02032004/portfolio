...
export default function AboutSection({ darkMode }: AboutSectionProps) {
  ...
  return (
    <motion.section
      id="about"
      ref={sectionRef}
      style={!isMobile ? { opacity, scale, y } : { opacity: 1, scale: 1, y: 0 }}
    ...
    >
      <div className="max-w-6xl mx-auto relative z-10 px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-16 gap-y-16 items-center">
          <motion.div
            initial={{ opacity: 0, x: isMobile ? 0 : -50, scale: isMobile ? 1 : 0.9 }}
            whileInView={{ opacity: 1, x: 0, scale: 1 }}
            viewport={{ once: true, margin: "-100px", amount: 0.3 }}
            transition={{ duration: 0.7, ease: "easeOut" }}
            className={`relative aspect-square rounded-2xl overflow-hidden mx-auto w-full max-w-sm md:max-w-lg group p-2 md:p-3 ${darkMode
              ? `bg-gradient-to-br from-gray-800/40 to-gray-900/30 ${isMobile ? 'shadow-lg' : 'shadow-xl shadow-cyan-900/30 hover:shadow-2xl hover:shadow-cyan-500/40'}`
              : `bg-gradient-to-br from-white/70 to-blue-50/50 ${isMobile ? 'shadow-lg' : 'shadow-xl shadow-blue-400/30 hover:shadow-2xl hover:shadow-cyan-500/40'}`
          ...
          >
            ...
            >
              {['React JS', 'Next JS', 'TypeScript', 'Tailwind CSS', 'MUI', 'Express JS', 'Git'].map((skill) => (
                <motion.span
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium shadow-sm cursor-default ${darkMode ? 'bg-gray-800 text-cyan-300 border border-gray-700' : 'bg-blue-50 text-blue-700 border border-blue-200'}`}
                >
                  {skill}
                </motion.span>
              ))}
            ...
            >
              ...
              >
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 transition-transform duration-300 ${!isMobile ? 'group-hover:translate-x-1' : ''}`} viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className={`absolute top-0 right-0 w-1/3 h-1/2 bg-gradient-to-bl ${darkMode ? 'from-cyan-900/15' : 'from-blue-100/60'} to-transparent opacity-50 ${!isMobile ? 'blur-3xl' : 'blur-xl'} -z-10`} />
    </motion.section>
  );
}
